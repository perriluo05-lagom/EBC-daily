# -*- coding: utf-8 -*-
"""规则解读层：基于真实数据用透明规则推导「解读」，仅作 LLM 不可用时的兜底。
...
# ---------------------------------------------------------------------------
# facts_text:导出结构化"数据事实"摘要（仅原始数字 + 来源，不含规则结论），供 LLM prompt 使用
# ---------------------------------------------------------------------------
# 仅复述真实采集数据，不新增任何数值，也不输出规则结论；LLM 只能引用此处出现的信息，
# 由 LLM 自行对数字做现象解读（规则结论仅作 render 兜底，不进入 LLM 输入）。
...
def _facts_overview(data: dict) -> list[str]:
...
def _facts_equity(data: dict) -> list[str]:
    eq = data.get("equity", {})
...
def _facts_cb(data: dict) -> list[str]:
...
def facts_text(data: dict, interp: dict | None = None) -> str:
    """导出五板块「数据事实」摘要（仅原始数字 + 来源，不含规则结论），供 LLM prompt 使用。

    LLM 只能引用此处出现的信息，以此守住「禁止编造」红线；解读由 LLM 自行完成。
    """
    lines = []
    lines += _facts_overview(data)
    lines += _facts_equity(data)
    lines += _facts_etf(data)
    lines += _facts_bond(data)
    lines += _facts_cb(data)
    return "\n".join(lines)
